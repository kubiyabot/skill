// AWS Skill - Using host configuration imports
// This demonstrates the CORRECT way to access configuration in WASM skills

// Host functions (getConfig, getAllConfig) are provided by the runtime
// and available as globals when using WIT imports
...
export function executeTool(toolName, argsJson) {
  try {
    ...
    // Get AWS credentials from host configuration
    const accessKeyId = getConfig('SKILL_AWS_ACCESS_KEY_ID') || '';
    const secretAccessKey = getConfig('SKILL_AWS_SECRET_ACCESS_KEY') || '';
    const region = getConfig('SKILL_AWS_REGION') || 'us-east-1';

    if (!accessKeyId || !secretAccessKey) {
      return JSON.stringify({
        err: 'AWS credentials not configured. Set SKILL_AWS_ACCESS_KEY_ID and SKILL_AWS_SECRET_ACCESS_KEY in instance configuration.'
      });
    }
...
export function validateConfig(configJson) {
  try {
    ...
    const requiredFields = ['SKILL_AWS_ACCESS_KEY_ID', 'SKILL_AWS_SECRET_ACCESS_KEY'];
    const missing = [];
